...
export const loader = async ({
    ...
    const res = await fetch(apiUrl({ endpoint: `store/category/${params.categoryId}` }));
    const jsonData = await res.json();
    if (params.categorySlug !== jsonData.category.slug) {
        return redirect(`/store/category/${params.categoryId}/${jsonData.category.slug}/`);
    }

    return json(jsonData);
};

export const meta: MetaFunction<typeof loader> = ({ data: categoryResponse }) => [
    {
