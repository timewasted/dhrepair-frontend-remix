...
export function CategoryListItem({
    ...
    return (
        <li className="flex py-1">
            {listContainsImages && (
                <div className={"w-28 h-28 flex-shrink-0 overflow-hidden" + (item.images.length > 0 ? " rounded-md border border-gray-200" : "")}>
                        ...
                        <div className="w-28 h-28 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                            <Link to={`/store/item/${item.id}/${item.slug}`}>
                                <img
                                    ...
                                    src={item.images[0].thumbWidth !== null && item.images[0].thumbHeight !== null ? (
            ...
            <div className="ml-4 flex flex-1 flex-col">
                <div>
                    <h3 className="font-semibold"><Link to={`/store/item/${item.id}/${item.slug}`}>{item.name}</Link></h3>
                    <p className="mt-2 text-sm">Part number: <Link to={`/store/item/${item.id}/${item.slug}`}>{item.sku}</Link></p>
                </div>
            </div>
        </li>
    );
